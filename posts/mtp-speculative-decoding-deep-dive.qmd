...
- **MTP 的 bouns token**：指的是最后一个预测中的下一个token可以直接接受， 比如MTP 1时，输入为 `[last, draft 0]` 此时预测输出为`[pred 0, pred 1]`， 当`pred 0 == draft 0`时，我们可以直接accept `pred 1`，这就是bouns token。 因为他是由主模型预测得到的，但我之前在这里踩过坑，实际上**bounds token和非mtp下串行预测出来的token会不一样**。